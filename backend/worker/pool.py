...
from backend.worker.process.base import SHUTDOWN_SIGNAL_MSG, BaseWorkerProcess
...
PROCESS_RESTART_PACEOUT_SECS = 5
...
class WorkerPoolSupervisor:
    ...
    def _is_worker_dead(self, p: Optional[Process], conn: Optional[Connection]) -> bool:
        if p is None:
            return True
        ...
        if conn.poll(HEARTBEAT_TIMEOUT_SECS):
            try:
                _ = conn.recv()
                return False
            ...
            while not self._shutdown.is_set():
                for worker_process_cls, num_workers in WORKER_PROCESS_CONFIGS:
                    for i in range(num_workers):
                        ...
                        dead = self._is_worker_dead(p, conn)
